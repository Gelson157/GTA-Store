...
addon = xbmcaddon.Addon('plugin.video.cdnplay')
...
communityfiles = os.path.join(profile, 'plugin.video.cdnplay')
...
CHBase = 'https://cdnplay01.blogspot.com/2020/01/categoria-addon.html'
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.cdnplay-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(On Demand Play 2.0,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(On Demand Play 2.0,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(On Demand Play 2.0,New source added.,5000,"+icon+")")
        if not url is None:
        ...
        for item in items:
            ...
            try:
                ...
                if len(item('link')) >0:
                    ...
                    for i in item('imdb'):
                        if not i.string == None:
                            if addon.getSetting('quasarorgenesis') == '0':
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('On Demand Play 2.0','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if not name in FAV:
                contextMenu.append(('Add to On Demand Play 2.0 Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from On Demand Play 2.0 Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to On Demand Play 2.0 Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#                print 'proxy',proxy
            ...
            xbmc.executebuiltin("XBMC.Notification(On Demand Play 2.0,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(On Demand Play 2.0,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(On Demand Play 2.0,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(On Demand Play 2.0,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(On Demand Play 2.0,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
